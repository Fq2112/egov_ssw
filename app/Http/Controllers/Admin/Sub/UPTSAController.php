...
class UPTSAController extends Controller
{
    ...
    {
        $now = Carbon::now();
        $tr_apotek = trPerizinanApotik::where('status',1)->orderBy('created_at','desc')->get();
        $tr_hama = trPerizinanHama::where('status',1)->orderBy('created_at','desc')->get();
        $tr_air = trPerizinanDepo::where('status',1)->orderBy('created_at','desc')->get();
...
//        dd($dthama,count($dtair),$dtapotek);
        ...
        return view('admin.sub.uptsa_dashboard', compact('dtapotek', 'dthama', 'dtair', 'dtsub', 'dtuser','c_apotik', 'c_air', 'c_hama', 'feedback', 'feedback_t', 'member', 'notif', 'role', 'admin'));
    }
}
